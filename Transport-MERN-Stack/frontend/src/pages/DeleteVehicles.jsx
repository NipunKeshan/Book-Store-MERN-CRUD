...
const DeleteVehicles = () => {
  ...
  const handleDeleteVehicle = async () => {
    try {
      setLoading(true);
      await axios.delete(`http://localhost:5555/vehicles/${id}`);
      setLoading(false);
      navigate('/vehicles');
    } catch (error) {
      setLoading(false);
  ...
  return (
    <div style={{ padding: '1rem' }}>
      ...
      {loading ? <Spinner /> : ''}
      <div style={{ backgroundColor:'white',border: '2px solid #4fd1c5', borderRadius: '10px', padding: '2rem', width: '600px', margin: 'auto' }}>
