...
const EditTransport = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios.get(`http://localhost:5555/transports/${id}`)
      .then((response) => {
  ...
  const handleEditTransport = () => {
    ...
    setLoading(true);
    axios
      .put(`http://localhost:5555/transports/${id}`, data)
      .then(() => {
        setLoading(false);
        navigate('/');
      })
  ...
  return (
    <div style={{ padding: '1rem' }}>
      ...
      {loading ? <Spinner /> : ''}
      <div style={{ backgroundColor:'white',border: '2px solid #4fd1c5', borderRadius: '10px', width: '600px', margin: 'auto', padding: '1rem' }}>
