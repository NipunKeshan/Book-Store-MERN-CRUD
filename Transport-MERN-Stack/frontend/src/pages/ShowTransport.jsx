...
const ShowTransport = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`http://localhost:5555/transports/${id}`)
      .then((response) => {
  ...
  return (
    <div style={{ padding: '1rem' }}>
      <BackButton />
      <h1 style={{ fontSize: '1.5rem', marginTop: '1rem' }}>Selected Transportation Details</h1><br></br>
      {loading ? (
        <Spinner />
      ) : (
        <table style={{ borderCollapse: 'collapse', backgroundColor:'white',border: '1px solid #ccc', borderRadius: '0.5rem', marginLeft: '25%' }}>
